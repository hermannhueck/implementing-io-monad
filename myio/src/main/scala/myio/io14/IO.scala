/*
  Step 14 leaves IO unchanged. The App defines 3 methods which return IO[A]: sumIO, fibonacciIO, factorialIO
  The App demonstrates, how these methods can be used in a for-comprehension.
 */
...
object IO {
  ...
  private case class Failed[A](io: IO[A]) extends IO[Throwable] {
    ...
    override def unsafeRun(): Throwable =
      try {
        io.unsafeRun()
